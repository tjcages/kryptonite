  ...
  renderMessages() {
    ...
    return messagesArray.map(messageCategory => {
      ...
      if (messageCategory.length) {
        category = messageCategory[0].labelIds.includes("CATEGORY_PROMOTIONS") ? "PROMOTIONS" : "T"
        category = messageCategory[0].labelIds.includes("CATEGORY_UPDATES") ? "UPDATES" : "I"
        category = messageCategory[0].labelIds.includes("CATEGORY_PERSONAL") ? "PERSONAL" : "S"
      }

      return (
        <div className="message-category">
        <HomeHeader key={category} category={category}/>
        {
