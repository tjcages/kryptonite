...
export class MessageContent extends Component {
    ...
    if (!emailMessageResult.loading) {
      if (!emailMessageResult.failed) {
        if (this.iframeRef.current) {
          ...
          body.onscroll = () => {
            this.updateScrollPosition(body.scrollTop)
          };
        }
      } else {
  ...
  updateScrollPosition(amount) {
    this.setState({scrollTop: amount})
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        />
        <div className="message-header" style={{height: `calc(100% -${this.state.scrollTop}px)`}}>
