...
export class MessageItem extends PureComponent {
  ...
  getMessage(evt) {
    ...
    // this.props.history.replace({ pathname: `/inbox/${this.props.data.id}`})
  ...
  render() {
    ...
    const currentMessageId = this.props.location.pathname.replace(this.props.match.url + "/", "")
    const selected = this.props.data.id === currentMessageId ? " selected" : "";
    ...
    return (
      <div 
        className={`message-row d-flex table-row-wrapper${selected} ${thread}`} 
        onClick={this.getMessage}
      >
        <div className="message-horizontal-row">
          <div className="message-image" />
          <div className={`table-row ${unread}`}>
            <NameSubjectFields 
              fromName={fromName} 
              subject={subject} 
              snippet={snippet} 
